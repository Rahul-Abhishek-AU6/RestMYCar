...
const { uploadProfilePicture, editProfile, blocking } = require('../../0619harisidh/api/controllers/updateControllers');
const { response } = require('../../0619harisidh/app');
...
async isAcceptedVehicle(req, res){
    try{
        ...
        if(vehicleOne.isAccepted) return res.send({error:"Job has been accepted or doesn't exist"})
...
async uploadProfilePicture(req, res){
    try{
        if(req.Owner) {var schema = Owner; user =Owner}
        ...
        response.status(202).send({message:"Profile picture uploaded successfully... You are looking awesome."})
    }catch(error){
        console.log(error)
        return res.status(500).send({error:error.message})
    }
...
// ---------------------------Changing Password (Jowner & customer------------------------------
async editPassword(req, res) {
    try {
        if (req.Owner) { var schema = Owner; user = req.Owner }
        if (req.Customer) { var schema = Custoemr; user = req.Customer }
...
async blocking(req, res){
    try{
        ...
        if(req.query.model === "Owner"){
            var schema = Carmodel;
            cosnt obj = await schema.updateMany({OwnerId: req.params.id}, {isBlocked: true})
        }
