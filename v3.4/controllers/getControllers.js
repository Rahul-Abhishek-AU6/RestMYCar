...
async vehicleById(req, res){
    try{
        const vehicle = await Carmodel.findOne({isAccepted: false, isBlocked:false})
...
async accountActivation(req, res){
    try{
        ...
        console.log('------176 controller/getCon-----', payload)
        if(payload){
            ...
            if(updated) return res.status(202).send({message:"Account activated Successfully. Please visit rentmecar.com and Login"});
