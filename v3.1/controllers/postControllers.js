...
function noOfVehicleOnRent(totalPosted){
...
module.exports = {
    //_________________Posting vehicle on rent by owner_________________
     ...
     async vehicleOnRent(req, res) {
         try{
        const rentVehicle = await new Carmodel({...req.body})
        console.log("Vehicle on rent", vehicleOnRent)
        vehicleOnRent.OwnerId = req.Owner._id;
        vehicleOnRent.OwnerEmail = req.Owner.email;
        vehicleOnRent.OwnerName = req.Owner.name;
        vehicleOnRent.save();

        const user = await Owner.findById(req.Owner._id)
        console.log(user)
        const totalPosted = noOfVehicleOnRent.findById(user.totalPosted);
        await Owner.findOneAndUpdate({id:req.Owner._id, isBlocked: false}, {totalPosted: totalPosted})
        console.log("---Your vehicle is on rent---")
        
        res.status(202).send({message:"---Your vehicle is on rent---"})
    }
...
async userLogin(req, res) {
    try{
        ...
        if(!email || !password)
            return res.status(400).send({error:"---Incorrect email or Password, try with correct one---"});
     ...
     //______________Defining Type of User_____________________
        ...
        if(!user) return res.status(400).send({error:"---Email is not found, Try again---"})
        ...
        if (!isMatched) return res.send({error:"---Incorrect password, have a look---"})
    //not verified--- make it happen on nodemailer   
        // if(!user.isVerified) return res.status(401).send({error:'---You are not verified, Kindly activate your account by clicking on activation link on your Email id---'})
        if(user.isBlocked) return res.status(401).send({error:'-- You are blocked, Kindly contact to admin department---'});

        const token = await jwt.sign({_id:user._id}, process.env.SECRET, {expiresIn: 1000* 1000* 6})
        user.jwt = token;
...
async forgotPassword(req, res){
    try{
        if(req.body.role == 'Owner') schema = Owner;
        if(req.body.role == "Customer") schema = Customer;
        if(!req.body.role) return res.send({error:"--=Incorrect credentials---"})

        const user = await schema.findOne({email:req.body.email, adhaarNumber:req.body.adhaarNumber})
        if(!user) return res.send({error: "---Incorrect creadential or Register again---"})
        if(user.isBlocked) return res.status(401).send({error:`${user.name}, You are blocked, ---for more details contact to rentmecar.com---`})

        const rawPassword = (Math.floor(Math.random()*100000000)).toString();
        user.hashedPassword = await hash(rawPassword, 10)
        user.password = hashedPassword;
        user.save();
        
        forgotPasswordMailing(req.body.email, rawPassword)
        return res.status(202).send({message:"---A system generated password is sent to your email, kindly visit your email---"});
